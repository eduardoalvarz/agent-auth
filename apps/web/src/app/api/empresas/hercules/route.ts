...
export async function GET(req: NextRequest) {
  ...
  // Enforce RLS-based access to 'hercules'
  ...
  if (error || !rows || rows.length === 0) {
    const res = new NextResponse(
      JSON.stringify({ error: "forbidden", message: "No autorizado para acceder a HÉRCULES" }),
  ...
  try {
    ...
    if (content == null) {
      const res = new NextResponse("No se pudo cargar el contenido de HÉRCULES.", { status: 404 });
      pendingCookies.forEach(({ name, value, options }) => res.cookies.set(name, value, options));
      return res;
    }
